  ...
  switch(action.type){
    case isActions.INITIALIZE_SCROLL:
      ...
      if(total-initSize<0){
        ...
        scrollDDI=(total-initSize)/addSize+(initSize-ctrlSize)/((initSize/ctrlSize)*total);
    ...
    case isActions.UPDATE_SCROLL:
      ...
      if(totalU-state.total>0){
        ...
        scrollDDUpd=(totalU-state.reachedDown-1)/state.addedSize+
        ...
        if(state.reachedDown>=totalU){
          ...
          scrollUDUpd=(state.reachedUp)/state.addedSize+
          (state.initialSize-state.controlSize)/((state.initialSize/state.controlSize)*totalU);
      ...
      console.log("ruUpd= ",ruUpd);
      console.log("rdUpd= ",rdUpd);
